...
validate.loginRules = () => {
return [
    // valid email is required and already exists in the DB
    body("account_email")
    .trim()
    .isEmail()
    .normalizeEmail() // refer to validator.js docs
    ...
    .custom(async (account_email) => {
      const emailExists = await accountModel.checkExistingEmail(account_email)
      if (!emailExists){
        throw new Error("Email doesn't exist. Please log in or use different email")
      }
    }),
...
/* ******************************
 * Check data and return errors or continue to update of the account
 * ***************************** */
